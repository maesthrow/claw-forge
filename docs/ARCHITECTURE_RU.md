...
ClawForge — надстройка над OpenClaw, которая автоматизирует создание AI-агентов через конвейер из четырёх специализированных агентов (analyst, developer, reviewer, tester). Пользователь описывает задачу главному агенту (architect) в Telegram, конвейер проектирует, разрабатывает, тестирует и деплоит нового агента в OpenClaw.
...
Пользователь → Architect → Конвейер:
  1. Analyst    — анализирует задачу, проверяет существующих агентов, формирует требования + тест-кейсы
  2. Developer  — генерирует SOUL.md, AGENTS.md, IDENTITY.md, skills, скрипты
  3. Reviewer   — статическая проверка: дубли, платформо-специфика, YAGNI (до 3 retry)
  → Deploy     — создаёт workspace, регистрирует в OpenClaw, устанавливает скрипты + зависимости
  4. Tester     — отправляет реальное тестовое сообщение задеплоенному агенту, оценивает ответ (до 2 retry, неблокирующий)
```
...
### Pipeline-агенты (analyst, developer, reviewer, tester)
...
│   ├── orchestration.py   — конвейер создания (analyst → developer → reviewer → tester)
...
│   ├── reviewer/          — SOUL.md, AGENTS.md ревьюера
...
2. Analyst получает задачу + список существующих агентов → возвращает JSON с decision + тест-кейсы (test_message, expected_behavior)
3. По decision: reuse → return, automation → create cron, create/extend → далее
4. Developer получает requirements → генерирует артефакты (SOUL.md, AGENTS.md, IDENTITY.md, skills, data_files, scripts, system_deps). При extend_existing также получает текущий SOUL.md агента.
5. Reviewer проверяет артефакты — дубли, платформо-специфика, YAGNI (до 3 retry с исправлениями developer-а)
6. Deploy: `create_new` → `create_agent_workspace()` + `register_agent()` + установка скриптов + зависимостей + heartbeat + gateway restart. `extend_existing` → `update_agent_files()` + скрипты + heartbeat update + gateway restart
7. Tester — отправляет test_message задеплоенному агенту через CLI, получает реальный ответ, оценивает по expected_behavior (до 2 retry с исправлениями developer-а). Неблокирующий: если тестер не одобрил после retry — агент всё равно задеплоен, замечания в уведомлении.
8. `format_notification()` формирует уведомление: результаты теста + замечания ревьюера + следующие шаги

**Cron-задачи:**
- Используют нативный формат OpenClaw: `kind: "cron"`, `expr: "0 6 * * *"`, `tz: "UTC"`
- Имя задачи совпадает с именем агента (например `currency_tracker`, не `currency_tracker-heartbeat`)
- `heartbeat_enabled`: true для публикаторов в каналы (cron сразу включён), false для агентов с подписками (агент включает при первом подписчике)
...
- `update` — обновляет файлы агентов, создаёт недостающих, удаляет старых pipeline-агентов (validator), чистит orphaned bindings из openclaw.json
- `uninstall` — удаляет всё: агентов, workspace, default workspace, bindings, cron jobs, реестр, логи. Никогда не удаляет токен бота архитектора (default/main).
...
Разделение ответственности: analyst решает **что** делать (новый/расширить/reuse), developer решает **как** (генерирует файлы), reviewer проверяет **качество артефактов** (статически), tester проверяет **реальное поведение** (запуск агента). Каждый специализирован и имеет чёткий формат ввода/вывода (JSON). Retry-логика позволяет developer исправлять ошибки по замечаниям reviewer и tester.
