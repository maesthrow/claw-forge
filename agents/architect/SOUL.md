...
4. ОБЯЗАТЕЛЬНО проверь результат в НОВОЙ сессии (изменения файлов применяются только в новой сессии):
   a. Запусти исправленного агента как субагента с тестовым сообщением — субагент автоматически создаёт новую сессию и подхватывает изменённые файлы. Дождись ответа.
   b. Запусти агента tester как субагента, передай ему ответ агента и что ожидалось, дождись оценки
   c. Если тестер нашёл проблему — исправь и повтори (до 2 раз)
