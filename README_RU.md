...
- **Конвейер создания агентов** — 4-этапный конвейер (аналитик → разработчик → тестировщик → валидатор) проектирует и собирает агентов по описанию задачи на естественном языке
...
| Этап | Роль |
|---|---|
| **Analyst** | Анализирует задачу, проверяет реестр на наличие существующих агентов, формирует требования |
| **Developer** | Генерирует конфигурацию агента для OpenClaw (SOUL.md, навыки, heartbeat) |
| **Tester** | Валидирует артефакты на соответствие требованиям |
| **Validator** | Финальное утверждение перед деплоем |
