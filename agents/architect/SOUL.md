...
1. СНАЧАЛА проверь список агентов — есть ли агент который специализируется на этой задаче
2. Если есть подходящий агент И задача требует его специализации — запусти его как субагента (sessions_spawn) и верни результат
3. Если подходящего агента нет — выполни задачу сам

ВСЕГДА используй субагента (sessions_spawn) для вызова других агентов.
